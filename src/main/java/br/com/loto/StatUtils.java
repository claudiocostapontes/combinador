...
public class StatUtils<T> {
    private final int cnt;
    private final T[] items;
    private final List<int[]> somatorios;

    public StatUtils(int cnt, T[] items) {
        this.cnt = cnt;
        this.items = items;
    ...
    public static void main(String[] args) {
        ...
        StatUtils<Integer> combinador = new StatUtils<>(tamanhoAposta, numerosLoteria);
        ...
        if (resposta.equalsIgnoreCase("S")) {
            System.out.println("Digite os 15 números sorteados (separados por vírgulas):");
            String linhaSorteados = scanner.next();
            String[] numerosSorteadosStr = linhaSorteados.split(",");
            Integer[] numerosSorteados = new Integer[15];

            if(numerosSorteadosStr.length == 15){
                for (int i = 0; i < 15; i++) {
                    numerosSorteados[i] = Integer.parseInt(numerosSorteadosStr[i].trim());
                }
            } else {
                System.out.println("Quantidade de números sorteados invalida, digite 15 números separados por vírgula.");
                return;
            }

            System.out.println("Digite os números da aposta para verificar a premiação (separados por vírgulas):");
            String linhaAposta = scanner.next();
            String[] apostaVerificacaoStr = linhaAposta.split(",");
            Integer[] apostaVerificacao = new Integer[tamanhoAposta];

            if(apostaVerificacaoStr.length == tamanhoAposta){
                for (int i = 0; i < tamanhoAposta; i++) {
                    apostaVerificacao[i] = Integer.parseInt(apostaVerificacaoStr[i].trim());
                }
            } else {
                System.out.println("Quantidade de números da aposta invalida, digite " + tamanhoAposta + " números separados por vírgula.");
                return;
            }
    ...
    public static void salvarApostas(List<Integer[]> apostas, String nomeArquivo) throws IOException {
        ...
        String caminhoArquivo = Paths.get(diretorioUsuario, nomeArquivo).toString();
